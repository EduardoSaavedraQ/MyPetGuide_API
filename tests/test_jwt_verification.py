...
from utils.supabase import get_jwks, get_supabase_client
import os
import requests
...
@pytest.mark.asyncio
async def test_supabase_jwt_decoding() -> None:
    try:
        key: dict[str, Any] | str = await get_jwks()
...
@pytest.mark.asyncio
async def test_supabase_jwt_is_expired() -> None:
    try:
        jwks: dict[str, Any] = await get_jwks()
...
@pytest.mark.asyncio
async def test_fresh_supabase_jwt() -> None:
...
@pytest.mark.asyncio
async def test_invalid_token_signature() -> None:

...
FAKE_EMAIL: str = f"test_usr@example.com"
...
SUPABASE_USER_IMAGE_PROFILES_PATH = "public/users"
...
def test_user_register_devuelve_respuestas_esperadas(
    ...
    try:
        ...
        if response.status_code != expected_status_code: print(response_data)

        assert response.status_code == expected_status_code

        if expected_status_code == 200:
            assert "jwt" in response_data and response_data["jwt"] is not None
            assert "id_profile" in response_data and response_data["id_profile"] is not None
            ...
            if image_path:
                ...
                supabase_response: list = (
                    ...
                    .list(SUPABASE_USER_IMAGE_PROFILES_PATH)
                )
